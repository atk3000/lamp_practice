...
//購入履歴に追加
function insert_ec_histories($db, $user_id)
{
  $sql = "
    INSERT INTO
    ec_histories(
...
//購入明細に追加
function insert_ec_details($db, $order_id, $item_id, $amount, $price)
{
  $sql = "
    INSERT INTO
    ec_details(
...
{
  if (insert_ec_histories($db, $carts[0]['user_id']) === false) {
    set_error('履歴データの作成に失敗しました。');
    return false;
  }
  $order_id = $db->lastInsertId();
  foreach ($carts as $cart) {
    if (insert_ec_details($db, $order_id, $cart['item_id'], $cart['amount'], $cart['price']) === false) {
